...
- for 출력: `10 7 4 1`
- (A) while 출력: `10 7 4 1` ✓
- (B) `i > 1`이므로 `1`을 출력하지 않음 ✗
- (C) 증감이 출력 전에 와서 순서 틀림 ✗
- (D) `i > 0`이므로 `1`을 출력하지만 우연히 같은 결과. 그러나 `i >= 1`과 `i > 0`은 정수에서 동치이므로 사실 (D)도 같은 출력. **하지만 원본 for문의 조건을 정확히 변환한 것은 (A).**
...
**Q14 정답 수정 안내:** 위의 상세 계산에서 정답은 **(D) 20**입니다.
